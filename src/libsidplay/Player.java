...
import static libsidplay.Config.PACKAGE_NAME;
import static libsidplay.Config.PACKAGE_VERSION;
import static libsidplay.Config.S_A_WHITE_EMAIL;
import static libsidplay.common.Event.event_phase_t.EVENT_CLOCK_PHI1;
import static libsidplay.common.ISID2Types.SID2_DEFAULT_OPTIMISATION;
import static libsidplay.common.ISID2Types.SID2_DEFAULT_POWER_ON_DELAY;
...
import static libsidplay.common.ISID2Types.SID2_MAX_POWER_ON_DELAY;
import static libsidplay.common.ISID2Types.sid2_clock_t.SID2_CLOCK_CORRECT;
import static libsidplay.common.ISID2Types.sid2_clock_t.SID2_CLOCK_NTSC;
import static libsidplay.common.ISID2Types.sid2_clock_t.SID2_CLOCK_PAL;
import static libsidplay.common.ISID2Types.sid2_env_t.sid2_envBS;
import static libsidplay.common.ISID2Types.sid2_env_t.sid2_envPS;
import static libsidplay.common.ISID2Types.sid2_env_t.sid2_envR;
...
import static libsidplay.common.ISID2Types.sid2_player_t.sid2_paused;
import static libsidplay.common.ISID2Types.sid2_player_t.sid2_playing;
import static libsidplay.common.ISID2Types.sid2_player_t.sid2_stopped;
import static libsidplay.common.ISID2Types.sid2_sample_t.SID2_LITTLE_SIGNED;
import static libsidplay.common.ISID2Types.sid2_sample_t.SID2_LITTLE_UNSIGNED;
import static libsidplay.common.SIDEndian.endian_16hi8;
import static libsidplay.common.SIDEndian.endian_little16;
import static libsidplay.components.mos6510.IOpCode.JMPi;
import static libsidplay.components.mos6510.IOpCode.JMPw;
import static libsidplay.components.mos6510.IOpCode.JSRw;
import static libsidplay.components.mos6510.IOpCode.LDAb;
import static libsidplay.components.mos6510.IOpCode.RTSn;
import static libsidplay.components.mos6510.IOpCode.STAa;
import static libsidplay.components.mos656x.MOS656X.mos656x_model_t.MOS6567R8;
import static libsidplay.components.mos656x.MOS656X.mos656x_model_t.MOS6569;
import static libsidplay.components.sidtune.SidTune.SIDTUNE_CLOCK_ANY;
import static libsidplay.components.sidtune.SidTune.SIDTUNE_CLOCK_NTSC;
import static libsidplay.components.sidtune.SidTune.SIDTUNE_CLOCK_PAL;
import static libsidplay.components.sidtune.SidTune.SIDTUNE_CLOCK_UNKNOWN;
import static libsidplay.components.sidtune.SidTune.SIDTUNE_COMPATIBILITY_BASIC;
import static libsidplay.components.sidtune.SidTune.SIDTUNE_COMPATIBILITY_PSID;
import static libsidplay.components.sidtune.SidTune.SIDTUNE_COMPATIBILITY_R64;
import static libsidplay.components.sidtune.SidTune.SIDTUNE_SIDMODEL_6581;
import static libsidplay.components.sidtune.SidTune.SIDTUNE_SIDMODEL_8580;
import static libsidplay.components.sidtune.SidTune.SIDTUNE_SIDMODEL_ANY;
import static libsidplay.components.sidtune.SidTune.SIDTUNE_SIDMODEL_UNKNOWN;
import static libsidplay.components.sidtune.SidTune.SIDTUNE_SPEED_CIA_1A;
import static libsidplay.components.sidtune.SidTune.SIDTUNE_SPEED_VBI;
...
import static libsidplay.mem.IPSIDDrv.PSIDDRV;
import static libsidplay.mem.IPowerOn.POWERON;
...
import libsidplay.common.SIDEndian;
...
import libsidplay.components.mos6510.C64Environment;
import libsidplay.components.mos6510.MOS6510;
import libsidplay.components.mos6510.SID6510;
import libsidplay.components.mos6526.C64CIA;
import libsidplay.components.mos6526.SID6526;
import libsidplay.components.mos656x.C64VIC;
import libsidplay.components.sidtune.SidTune;
import libsidplay.components.sidtune.SidTuneInfo;
import libsidplay.components.xsid.C64XSID;
...
public class Player extends C64Env /* extends C64Environment */{
	...
	* gives us about 1k samples per mixing event on typical settings. */
	private static final int MIXER_EVENT_RATE = 20000;
	 ...
	 * 10 credits max
	 */
	private static String credit[] = new String[12];
	...
	private class EventRTC extends Event {
	...
	public short /* uint8_t */m_ram[], m_rom[];
	...
	public long /* int_least32_t */m_leftVolume;

	public long /* int_least32_t */m_rightVolume;
	...
	private double m_cpuFreq = CLOCK_FREQ_PAL;
	...
	// for n samples
	long /* event_clock_t */ m_rtcClock;
	long /* event_clock_t */ m_rtcPeriod;
	...
	private void fakeIRQ() {
		...
		cpu.triggerIRQ();
		cpu.reset (playAddr, (short) 0, (short) 0, (short) 0);
	}
	...
	private void mixer() {
        ...
        while (i < samples) {
            ...
            i += j;
            sample1 = (int /* uint_least16_t */) (sample1 * m_leftVolume / VOLUME_MAX);
            sample1 /= j;
            sample2 = (int /* uint_least16_t */) (sample2 * m_leftVolume / VOLUME_MAX);
            sample2 /= j;
            /* mono mix. */
            ...
            SIDEndian.endian_16(buf, bufPos, sample1);
            bufPos += 2;
            m_sampleIndex += 2;
            if (m_cfg.playback == sid2_stereo) {
            	SIDEndian.endian_16(buf, bufPos, sample2);
                bufPos += 2;
                m_sampleIndex += 2;
            }
        }
	...
	void mute(int sidNum, int voice, boolean enable) {
		if (sid[sidNum] != null) {
	...
	private void digiBoost8580(boolean digiBoost) {
		sid[0] = xsid.emulation();
		for (int i = 0; i < SID2_MAX_SIDS; i++)
			sid[i].digiBoost(digiBoost);
		sid[0] = xsid;
	}
	...
	private void envReset(boolean safe) {
		...
		if (m_info.environment != sid2_envR) {
			short /* uint8_t */song = (short) ((m_tuneInfo.currentSong - 1) & 0xff);
			short /* uint8_t */bank = iomap(m_tuneInfo.initAddr);
			evalBankSelect(bank);
			m_playBank = iomap(m_tuneInfo.playAddr);
			if (m_info.environment != sid2_envPS)
				cpu.reset (m_tuneInfo.initAddr, song, (short) 0, (short) 0);
			else
	...
	private int psidDrvReloc(SidTuneInfo tuneInfo, sid2_info_t info) {
		...
		{ // Place psid driver into ram
			...
			if (tuneInfo.compatibility == SIDTUNE_COMPATIBILITY_BASIC) {
				...
				short /* uint8_t */prg[] = {
						LDAb,
						(short /* uint8_t */) ((tuneInfo.currentSong - 1) & 0xff),
		...
		{ // Setup the Initial entry point
			...
			// Tell C64 about song
			addr[pos++] = (short /* uint8_t */) ((tuneInfo.currentSong - 1) & 0xff);
	...
	public Player() {
		...
		cpu = new SID6510(m_scheduler);
		xsid = new C64XSID(this, nullsid = new NullSID());
		C64CIA ciaSupport = new C64CIA();
		cia = ciaSupport.new c64cia1(this);
		cia2 = ciaSupport.new C64cia2(this);
		sid6526 = new SID6526(this);
		vic = new C64VIC(this);
		m_mixerEvent = new Event("Mixer") {
		...
		m_cfg.sampling = sampling_method.SAMPLE_FAST;
		...
		credit[0] = "Java port and Applet/JFrame GUI:\n" +
				"\tCopyright (C) 2007 Ken Händel\n" +
				"\thttp://sourceforge.net/projects/jsidplay2/";
		credit[1] = "Drag and Drop support: Robert Harder\n" +
		"\thttp://iharder.sourceforge.net";
		credit[2] = "graphical output: Joakim Eriksson\n" +
		"\thttp://sourceforge.net/projects/jac64/";
		credit[3] = "Testing and Feedback: Nata of proNoise";
		credit[4] = "Distortion Simulation: Antti Lankila\n" +
		"\thttp://bel.fi/~alankila/c64-sw/";
		credit[5] = "HVSC playroutine identity scanner (SIDId V1.07):\n" +
				"\tCopyright (C) 2007 Lasse Öörni";
		credit[6] = PACKAGE_NAME + " V" + PACKAGE_VERSION
				+ " Engine:\n\tCopyright (C) 2000 Simon White <"
				+ S_A_WHITE_EMAIL + ">\n"
				+ "\thttp://sidplay2.sourceforge.net";
		credit[7] = xsid.credits();
		credit[8] = "*MOS6510 (CPU) Emulation:\n" +
				"\tCopyright (C) 2000 Simon White <"
				+ S_A_WHITE_EMAIL + ">";
		credit[9] = cia.credits();
		credit[10] = vic.credits();
		credit[11] = null;
	}
	...
	public int config(final sid2_config_t cfg) {
		...
		if (m_tune != null && m_tune.bool()) {
			...
			if (m_playerState != sid2_paused) {
				...
				for (int i = 0; i < SID2_MAX_SIDS; i += 1)
					sid[i].sampling((long)cpuFreq, cfg.frequency, cfg.sampling);
		...
		if (m_tuneInfo.sidChipBase2 != 0 || cfg.forceStereoTune) {
			// change playback to stereo if stereo tune was detected or forced
			cfg.playback = sid2_stereo;
			if (m_tuneInfo.sidChipBase2 != 0) {
				// tune tells us where the second SID is located
				// Assumed to be in d4xx-d7xx range
				m_sidmapper[(m_tuneInfo.sidChipBase2 >> 5) & (SID2_MAPPER_SIZE - 1)] = 1;
			} else {
				// Dont know where the second SID is located, take parametrized base
	...
	public int fastForward(int percent) {
	    ...
	    if (m_fastForwardFactor > 1) {
	        sid[0].optimisation((short) 2);
	        sid[1].optimisation((short) 2);
	    } else {
	        sid[0].optimisation(m_cfg.optimisation);
	        sid[1].optimisation(m_cfg.optimisation);
	     }
		return 0;
	}
	...
	final long /* int_least32_t */VOLUME_MAX = 255;
	...
	private class file65 {
		...
		short /* unsigned char */[] buf;

		int tbase, tlen, dbase, dlen, bbase, blen, zbase, zlen;

		int tdiff, ddiff, bdiff, zdiff;

		short /* unsigned char */[] segt;

		short /* unsigned char */[] segd;

		short /* unsigned char */[] utab;

		short /* unsigned char */[] rttab;

		short /* unsigned char */[] rdtab;

		short /* unsigned char */[] extab;
	};
	...
	private BufPos reloc65(short /* unsigned char** */[] buf,
		...
		int rdtabPos = reloc_seg(file.segt, segtPos, file.tlen, file.rttab,
				rttabPos, file);
		int extabPos = reloc_seg(file.segd, sehdPos, file.dlen, file.rdtab,
				rdtabPos, file);
