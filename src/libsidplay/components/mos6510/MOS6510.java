...
import static libsidplay.common.SIDEndian.endian_16;
import static libsidplay.common.SIDEndian.endian_16hi8;
import static libsidplay.common.SIDEndian.endian_16lo8;
import static libsidplay.common.SIDEndian.endian_32hi8;
import static libsidplay.common.SIDEndian.endian_32lo16;
import static libsidplay.common.SIDEndian.endian_32lo8;
import static libsidplay.components.mos6510.IConf6510.MOS6510_ACCURATE_CYCLES;
...
public class MOS6510 extends C64Environment /* extends Event */{
	...
	protected void Initialise() {
		...
		m_blocked = false;
		event.schedule(eventContext, 0, m_phase);
	}
	...
	protected void dcm_instr() {
		...
		PutEffAddrDataByte();
		Cycle_Data = (short) (--Cycle_Data & 0xff);
	...
	protected void dec_instr() {
		PutEffAddrDataByte();
		setFlagsNZ(Cycle_Data = (short) (--Cycle_Data & 0xff));
	}

	protected void dex_instr() {
		setFlagsNZ(Register_X = (short) (--Register_X & 0xff));
		clock();
	}

	protected void dey_instr() {
		setFlagsNZ(Register_Y = (short) (--Register_Y & 0xff));
		clock();
	}
	...
	public void DumpState() {
		...
		m_fdbg.append(String.format("\n%5d :", line++));
		m_fdbg.append(String.format(
				" PC  I  A  X  Y  SP  DR PR NV-BDIZC  Instruction (%d)\n",
				m_dbgClk));
		m_fdbg.append(String.format("       %04x ", instrStartPC));
		m_fdbg.append(String.format("%d ", interrupts.irqs));
		m_fdbg.append(String.format("%02x ", Register_Accumulator));
		m_fdbg.append(String.format("%02x ", Register_X));
		m_fdbg.append(String.format("%02x ", Register_Y));
		m_fdbg.append(String.format("01%02x ",
				endian_16lo8(Register_StackPointer)));
		m_fdbg.append(String.format("%02x ", envReadMemDataByte(0)));
		m_fdbg.append(String.format("%02x ", envReadMemDataByte(1)));
		...
		case RRAz:
			...
			m_fdbg.append(String.format("%02x    ",
					(short /* uint8_t */) operand));
			break;
		// Zero Page with X Offset Addressing Mode Handler
		case ADCzx:
		case ANDzx:
		...
		case LAXzy:
			...
			m_fdbg.append(String.format("%02x    ", endian_16lo8(operand)));
			break;
		// Absolute Addressing Mode Handler
		case ADCa:
		case ANDa:
		...
		case RRAa:
			...
			m_fdbg.append(String.format("%02x %02x ", endian_16lo8(operand),
					endian_16hi8(operand)));
			break;
		...
		case RRAz:
			// ASOz AXSz DCMz INSz LSEz - Optional Opcode Names
			m_fdbg.append(String.format("z  %02x {%02x}",
					(short /* uint8_t */) operand, data));
			break;
		case SAXz:
		case STAz:
		case STXz:
		...
		case NOPz_2:
			...
			m_fdbg.append(String.format("z  %02x", endian_16lo8(operand)));
			break;
		...
		case SREzx:
			// ASOzx DCMzx INSzx LSEzx - Optional Opcode Names
			m_fdbg.append(String.format("zx %02x,X", endian_16lo8(operand)));
		...
		case LDXzy:
			// AXSzx - Optional Opcode Names
			m_fdbg.append(String.format("zy %02x,Y", endian_16lo8(operand)));
			m_fdbg.append(String.format(" [%04x]{%02x}", address, data));
			break;
		case STXzy:
		case SAXzy:
		...
		case NOPb_4:
			...
			m_fdbg.append(String.format("b  #%02x", endian_16lo8(operand)));
			break;
		...
		case BVCr:
		case BVSr:
			m_fdbg.append(String.format("r  #%02x", endian_16lo8(operand)));
			m_fdbg.append(String.format(" [%04x]", address));
			break;
		...
		case RRAix:
			// ASOix AXSix DCMix INSix LSEix - Optional Opcode Names
			m_fdbg.append(String.format("ix (%02x,X)", endian_16lo8(operand)));
			m_fdbg.append(String.format(" [%04x]{%02x}", address, data));
			break;
		case SAXix:
		case STAix:
		...
		case SREiy:
			// AXAiy ASOiy LSEiy DCMiy INSiy - Optional Opcode Names
			m_fdbg.append(String.format("iy (%02x),Y", endian_16lo8(operand)));
			m_fdbg.append(String.format(" [%04x]{%02x}", address, data));
			break;
		case SHAiy:
		case STAiy:
	...
	public void aecSignal(boolean state) {
		if (aec != state) {
			...
			event.schedule(eventContext, eventContext.phase() == m_phase ? 1
					: 0, m_phase);
		}
	}
