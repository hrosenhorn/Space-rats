...
public abstract class XSID /* extends Event */extends SIDEmu {
	...
	public class Channel {
		...
		private Channel(final String name, IEventContext context, XSID xsid) {
			...
			m_xsid = xsid;
			m_sampleEvent = new Event("xSID Sample") {
			...
			m_galwayEvent = new Event("xSID Galway") {
		...
		private void silence() {
			...
			m_xsid.event.schedule(m_context, 0, m_phase);
		}
		...
		private void sampleInit() {
			...
			if (XSID.isLoggable(Level.INFO)) {
				if (XSID.isLoggable(Level.FINE)) {
					XSID.fine(String.format(
							"XSID [%s]: Sample Start Address:  0x%04x", m_name,
							address));
					XSID.fine(String.format(
							"XSID [%s]: Sample End Address:    0x%04x", m_name,
							samEndAddr));
					XSID.fine(String.format(
							"XSID [%s]: Sample Repeat Address: 0x%04x", m_name,
							samRepeatAddr));
					XSID.fine(String.format("XSID [%s]: Sample Period: %d",
							m_name, samPeriod));
					XSID.fine(String.format("XSID [%s]: Sample Repeat: %d",
							m_name, samRepeat));
					XSID.fine(String.format("XSID [%s]: Sample Order: %d",
							m_name, samOrder));
				}
					...
					if (XSID.isLoggable(Level.INFO)) {
						XSID
								.log(Level.FINE, String
										.format(
												"XSID [%s]: Sample Stop (%1d Cycles, %1d Outputs)",
		...
		private void galwayClock() {
			if (--galLength != 0)
				...
				if (XSID.isLoggable(Level.INFO)) {
					XSID.log(Level.FINE, String.format(
							"XSID [%s]: Galway Stop (%1d Cycles, %1d Outputs)",
							m_name, cycles, outputs));
					if (status != 0xfd) {
						XSID
		...
		private void galwayTonePeriod() {
			...
			if (XSID.isLoggable(Level.INFO)) {
				...
				XSID.log(Level.FINE, String.format(
						"XSID [%s]: Length %d, LoopWait %d, NullWait %d",
						m_name, galLength, galLoopWait, galNullWait));
				XSID.log(Level.FINE, String.format("XSID [%s]: Tones %d, Data %d", m_name,
						galTones, m_xsid.readMemByte(address + galTones)));
			}
...
//			0x7f };
	...
	private void setSidData0x18() {
		...
		if (XSID.isLoggable(Level.INFO)) {
			if ((sampleOffset + sampleOutput()) > 0x0f) {
				XSID.log(Level.FINE, String.format(
						"XSID: Sample Wrapped [offset %d, sample &d]",
						sampleOffset, sampleOutput()));
			}
			if (XSID.isLoggable(Level.FINE)) {
				XSID.fine(String.format(
						"XSID: Writing Sample to SID Volume [0x%02x]", data));
			}
		}
	...
	private void sampleOffsetCalc() {
		...
		if (XSID.isLoggable(Level.INFO)) {
			...
			XSID.log(Level.FINE, String
					.format("XSID: Sample Offset %d based on channel(s) ",
							sampleOffset) + channel);
		}
	...
	public void write(int /* uint_least16_t */addr, short /* uint8_t */data) {
		...
		if (XSID.isLoggable(Level.FINE)) {
			XSID.fine(String.format("XSID: Addr 0x%02x, Data 0x%02x", tempAddr,
					data));
		}
