...
public abstract class MOS6526 implements IComponent {
	 ...
	 */
	private final String credit = "*MOS6526 (CIA) Emulation:\n"
			+ "\tCopyright (C) 2001-2004 Simon White <" + S_A_WHITE_EMAIL + ">";

	public short /* uint8_t */regs[] = new short[0x10];
	...
	public IEventContext event_context;

	public event_phase_t m_phase;
	...
	protected MOS6526(IEventContext context, int count) {
		idr = 0;
		event_context = (context);
		m_phase = event_phase_t.EVENT_CLOCK_PHI1;
		m_todPeriod = ~0 & 0xffffffffl; // Dummy
		m_taEvent = new Event("CIA Timer A") {
		...
		m_tbEvent = new Event("CIA Timer B") {
		...
		m_todEvent = new Event("CIA Time of Day") {
	...
	protected void ta_event() {
		...
		if ((cra & 0x08) != 0) {
			...
			// Reset event
			m_taEvent.schedule(event_context, (long /* event_clock_t */) ta + 1, m_phase);
		}
	...
	protected void tb_event() {
		...
		if ((crb & 0x08) != 0) {
			...
			// Reset event
			m_tbEvent.schedule(event_context, (long /* event_clock_t */) tb + 1, m_phase);
		}
	...
	public void write(short /* uint_least8_t */addr, short /* uint8_t */data) {
		...
		case CRA:
			...
			if ((data & 0x21) == 0x01) {
				// Active
				m_taEvent.schedule (event_context, (long /* event_clock_t */) ta + 3, m_phase);
			} else {
				// Inactive
				m_taEvent.cancel ();
			}
		...
		case CRB:
			...
			if ((data & 0x61) == 0x01) {
				// Active
				m_tbEvent.schedule(event_context, (long /* event_clock_t */) tb + 3, m_phase);
			} else {
				// Inactive
				m_tbEvent.cancel();
			}
	 ...
	 *         This is the same as have different clock freqs connected to pins
	 *         on the IC.
	 * @param clock
	 */
