...
import static libsidplay.components.sidtune.ISidTuneCfg.SIDTUNE_R64_MIN_LOAD_ADDR;
...
import libsidplay.components.sidtune.Mus.Voice3Index;
...
public class SidTune
{
	 ...
	 * @param fileNameExt
	 */
	public void setFileNameExtensions(final String[] fileNameExt)
	{
		fileNameExtensions = ((fileNameExt != null) ? fileNameExt
				: defaultFileNameExt);
	}
			...
			final int /* uint_least16_t */selectedSong)
	{
		...
		if (info.compatibility == SIDTUNE_COMPATIBILITY_R64)
			...
			{ // This
				...
				// SPEED
				// value correctly, like it is described in the PlaySID
				// documentation.
		...
		if (status && c64buf != null)
		{
			...
			if (endPos <= SIDTUNE_MAX_MEMORY)
			{
				...
				for (int i = 0; i < info.c64dataLen; i += 16)
				{
					...
					{
						TUNE.fine(String.format("0x%02x ", c64buf[info.loadAddr
								+ i + j]));
					}
					TUNE.fine("\n");
				}
	...
	/**
	 * @param destFileName
	 * @param overWriteFlag true = Overwrite existing file, false = Default<BR>
							...
							Buffer_sidtt /* Buffer_sidtt<const uint_least8_t>& */bufferRef)
	{
		...
		try
		{
			if (!fileBuf.assign(new short [fileLen],
					fileLen))
			{
	 ...
	 */
	public static final String defaultFileNameExt[] = new String[] {
			...
			else
			{
				...
				if (ret == LoadStatus.LOAD_NOT_MINE)
				{
					...
					if (ret != LoadStatus.LOAD_NOT_MINE)
					{
						...
						// try to load a MUS stereo tune (<name>_A.mus / <name>_B.mus)
						if (fileName.toLowerCase().endsWith("_a.mus")
								|| fileName.toLowerCase().endsWith("_b.mus"))
						{
							int ab = (fileName
									.charAt(fileName.length() - 5));
							fileName2.delete(0, fileName2.length());
							fileName2
									.append(
											fileName.substring(0, fileName
													.length() - 6)).append("_")
									.append((char) (ab + (ab == 65 || ab == 97 ? 1 : -1))).append(".mus");
							if (loadFile(fileName2.toString(), fileBuf2))
							{
								// Check if tunes in wrong order and therefore
								// swap them here
								if (ab == 65 || ab == 97)
							   ...
							   final int /* uint_least32_t */bufferLen)
	{
		...
		short[] /* uint_least8_t* */tmpBuf;
		if (null == (tmpBuf = new short /* uint_least8_t */[bufferLen]))
		{
			info.statusString = txt_notEnoughMemory;
			return;
		}
								  ...
								  Buffer_sidtt /* Buffer_sidtt<const uint_least8_t>& */buf)
	{
		...
		{
			info.path = new String(dataFileName);
			...
			final String sourceName, final String sourceExt)
	{
		if (destString.length() > 0)
			destString.delete(0, destString.length());
