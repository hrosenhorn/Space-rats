...
import static libsidplay.common.SIDEndian.endian_big16;
import static libsidplay.common.SIDEndian.endian_big32;
import static libsidplay.components.sidtune.ISidTuneCfg.SIDTUNE_PSID2NG;
...
import static libsidplay.components.sidtune.SidTune.SIDTUNE_COMPATIBILITY_C64;
...
import static libsidplay.components.sidtune.SidTune.SIDTUNE_MAX_SONGS;
...
import static libsidplay.components.sidtune.SidTune.LoadStatus.LOAD_ERROR;
import static libsidplay.components.sidtune.SidTune.LoadStatus.LOAD_NOT_MINE;
import static libsidplay.components.sidtune.SidTune.LoadStatus.LOAD_OK;

import java.io.IOException;
import java.io.OutputStream;

import libsidplay.components.sidtune.SidTune.LoadStatus;
...
public class PSid {
	...
	protected final LoadStatus PSID_fileSupport(
		...
		sidtune.infoString[0] = new String(pHeader.name, 0, Math.min(
				i, _sidtune_psid_maxStrLen));
		info.infoString[0] = new String(sidtune.infoString[0]);
		// Author
		...
		sidtune.infoString[1] = new String(pHeader.author, 0, Math.min(
				i, _sidtune_psid_maxStrLen));
		info.infoString[1] = new String(sidtune.infoString[1]);
		// Released
		...
		sidtune.infoString[2] = new String(pHeader.released, 0, Math.min(
				i, _sidtune_psid_maxStrLen));
		info.infoString[2] = new String(sidtune.infoString[2]);
	...
	protected boolean PSID_fileSupportSave(OutputStream fMyOut,
		...
		try {
			...
			short /* uint_least32_t */speed = 0, check = 0;
