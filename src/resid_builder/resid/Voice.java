...
public class Voice {
	 ...
	 */
	protected float /* sound_sample */wave_zero;

	/**
	 * Multiplying D/A DC offset.
	 */
	protected float /* sound_sample */voice_DC;
	...
	float output() {
		...
		float w = wave.output();
		float e = envelope.output();
		float we = w * e;

		/*
		 * voice_DC overloaded to test for 6581. the waveform at this point runs
		 * from 0 to fff and env from 0 to ff. quick measurements indicate
		 * slight waveshaping that gets worse if the output volume increases.
		 * There's also plenty of evidence of FC-like kinkiness in the waveform
		 * DACs, the output basically jumps up and down as the bits get turned
		 * on and off in the output. It's a mess. Curiously, the problem gets
		 * less worse as the envelope value grows. It must be a bug in the
		 * implementation of the multiplying DAC.
		 */
		if (voice_DC != 0)
			we -= we * we / (4095.f * 255.f * 2.25f) / 2.f;

		return we - wave_zero * e + voice_DC;
	}
