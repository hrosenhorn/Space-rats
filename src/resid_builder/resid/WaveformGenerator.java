...
public class WaveformGenerator {
	...
	public void set_chip_model(chip_model model) {
		if (model == chip_model.MOS6581) {
			ST_lockup = true;
			wave__ST = Wave6581__ST.wave6581__ST;
			wave_P_T = Wave6581_P_T.wave6581_P_T;
			wave_PS_ = Wave6581_PS_.wave6581_PS_;
			wave_PST = Wave6581_PST.wave6581_PST;
		} else {
			ST_lockup = false;
			wave__ST = Wave8580__ST.wave8580__ST;
			wave_P_T = Wave8580_P_T.wave8580_P_T;
			wave_PS_ = Wave8580_PS_.wave8580_PS_;
			wave_PST = Wave8580_PST.wave8580_PST;
		}
	}
	...
	public void writeCONTROL_REG(int /* reg8 */control) {
		...
		if (test_next != 0 && test == 0) {
			...
			shift_register = (shift_register & 0x7ffffd)
					| ((bit19 ^ 1) << 1);
			noise_overwrite_delay = 2000000; /* 2 seconds */
		}
		// Test bit cleared.
	...
	public final void clock() {
		...
		/*
		 * The combined waveform with ring mod seems to destroy the highest 2 bits of
		 * the oscillator. Why this happens is not clear, but it probably involves
		 * coupling of the highest bits via the waveform selector and the XOR mechanism
		 * that produces triangle waveform. This only affects 6581. It's probable that
		 * the sync source must be in 0 state as well.
		 */
		if (ST_lockup && ((waveform & 1) != 0) && ((waveform & 2) != 0)
				&& (ring_mod != 0) && (sync_source.accumulator & 0x800000) == 0)
	 ...
	 * @return triangle
	 */
	protected final int /* reg12 */output___T() {
	 ...
	 * @return sawtooth
	 */
	protected final int /* reg12 */output__S_() {
	 ...
	 * @return
	 */
	protected final int /* reg12 */output_P__() {
	 ...
	 */
	protected final int /* reg12 */outputN___() {
	 ...
	 */
	protected final int /* reg12 */output__ST() {
	 ...
	 * sample. The sample is output if the pulse output is on.
	 */
	protected final int /* reg12 */output_P_T() {
		/*
		 * ring modulation does something odd with this waveform. But I don't
	 ...
	 * sample. The sample is output if the pulse output is on.
	 */
	protected final int /* reg12 */output_PS_() {
	 ...
	 * sample. The sample is output if the pulse output is on.
	 */
	protected final int /* reg12 */output_PST() {
	 ...
	 */
	protected final int /* reg12 */outputN__T() {
		return 0;
	}
	 ...
	 * We hope that nobody is actually using it.
	 */
	protected final int /* reg12 */outputN_S_() {
		return 0;
	}
	 ...
	 * We hope that nobody is actually using it.
	 */
	protected final int /* reg12 */outputN_ST() {
		return 0;
	}
	 ...
	 * We hope that nobody is actually using it.
	 */
	protected final int /* reg12 */outputNP__() {
		return 0;
	}
	 ...
	 * We hope that nobody is actually using it.
	 */
	protected final int /* reg12 */outputNP_T() {
		return 0;
	}
	 ...
	 * We hope that nobody is actually using it.
	 */
	protected final int /* reg12 */outputNPS_() {
		return 0;
	}
	 ...
	 * We hope that nobody is actually using it.
	 */
	protected final int /* reg12 */outputNPST() {
		return 0;
	}
