...
public class SID {
	...
	/* These constants assume no loss of quality arising from quantizing the
	* FIR table. However, there's a significant penalty for this. A spectrum
	* of 17-bit FIR table shows noise floor around -84 dB, and each lost bit
	* reduces the accuracy by 6 dB. With a 15-bit table the SNR is only -76 dB.
	*
	* This allows planning for only 13-bit audio, instead of 16-bit audio.
	*
	...
	*/
	...
	// Joakim: added for UI accesibility
	private int[] sidMem = new int[32];

	public boolean fDigiBoost;

	/**
	 * Constructor.
	 */
	public SID(int count) {
		// Initialize pointers.
		sample = null;
		fir = null;
	...
	public void set_distortion_properties(boolean en, float r, float lp) {
		filter.set_distortion_properties(en, r, lp);
	}
	 ...
	 */
	public float output() {
		final int range = 1 << 15;
		return extfilt.output() / (float) ((4095*255 >> 7)*3*15*2 / range);
	}
	...
	public void write(int /* reg8 */offset, int /* reg8 */value) {
		...
		// Joakim: added for UI accessibility
		sidMem[offset & 0x1f] = value;
	...
	public boolean set_sampling_parameters(double clock_freq,
		 ...
		 * frequencies below ~ 44.1kHz, and 20 kHz for higher sample frequencies. */
		if (pass_freq < 0) {
			pass_freq = 20000;
		...
		sampling = method;
		filter.set_clock_frequency((float)clock_freq);
		extfilt.set_clock_frequency((float)clock_freq);
		...
		// 16 bits -> -96 dB stopband attenuation. 13 bits -> -78 dB.
		final double A = -20*Math.log10(1.0/(1 << 13));
		...
		// The filter order will maximally be ~60 with the current constraints.
		// N >= (96.33 - 7.95)/(2.285*0.2*pi) -> N >= 60
		...
		for (int i = 0; i < fir_RES; i++) {
			...
			for (int j = -fir_N / 2; j <= fir_N / 2; j++) {
				...
				double val = filter_scale*f_samples_per_cycle*wc/pi*sincwt*Kaiser;
				fir[fir_offset + j] = (float)val;
			}
		}
	 ...
	 * @param delta_t
	 */
	private final void clock(int /* cycle_count */delta_t) {
		int i;
	 ...
	 * @return
	 */
	public final int clock(int /* cycle_count */ delta_t, short buf[], int pos, int n, int interleave) {
		switch (sampling) {
		default:
		case SAMPLE_FAST:
			return clock_fast(delta_t, buf, pos, n, interleave);
		case SAMPLE_INTERPOLATE:
			return clock_interpolate(delta_t, buf, pos, n, interleave);
		case SAMPLE_RESAMPLE_INTERPOLATE:
			return clock_resample_interpolate(delta_t, buf, pos, n, interleave);
		case SAMPLE_RESAMPLE_FAST:
			return clock_resample_fast(delta_t, buf, pos, n, interleave);
		}
	}
	 ...
	 * @return
	 */
	public final int clock_fast(int /* cycle_count */ delta_t, short buf[], int pos, int n,
			int interleave) {
		int s = 0;
	 ...
	 * @return
	 */
	private final int clock_interpolate(int /* cycle_count */ delta_t, short buf[], int pos, int n,
			int interleave) {
		int s = 0;
		int i;
			...
			if (s >= n) {
				...
				float v = sample_prev + (sample_offset*(sample_now - sample_prev) / (1 << FIXP_SHIFT));
				// Saturated arithmetics to guard against 16 bit sample overflow.
				final int half = 1 << 15;
				if (v >= half) {
					v = half - 1;
	...
	static float convolve(final float a[], int aPos, final float b[], int bPos, int n) {
		float out = 0.f;
		for (int i = 0; i < n; i++)
			out += (a[aPos+i]) * (b[bPos+i]);
		return out;
	}
	 ...
	 * @return
	 */
	private final int clock_resample_interpolate(int /* cycle_count */ delta_t, short buf[],int pos, 
		...
		int s = 0;
			...
			if (s >= n) {
				...
				float fir_offset_rmd = ((sample_offset * fir_RES) & FIXP_MASK) / (1 << FIXP_SHIFT);
				int sample_start = sample_index - fir_N + RINGSIZE;
				 
				float v1 = convolve(sample, sample_start, fir, fir_offset * fir_N, fir_N);
				...
				float v2 = convolve(sample, sample_start, fir, fir_offset * fir_N, fir_N);
	 ...
	 * @return
	 */
	private final int clock_resample_fast(int /* cycle_count */ delta_t, short buf[], int pos, int n,
			int interleave) {
		int s = 0;
		...
		for (;;) {
			...
			float v = convolve(sample, sample_start, fir, fir_offset * fir_N, fir_N);
