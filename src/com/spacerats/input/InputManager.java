package com.spacerats.input;

import com.spacerats.config.ConfigManager;
import com.spacerats.wii.SpaceMote;
...
public class InputManager implements KeyListener, MouseListener, MouseMotionListener, MouseWheelListener
{
	...
	private Thread spaceMoteThread;
	...
	public InputManager(Component comp)
	{
		...
		if (ConfigManager.getInstance().getWiimote())
		{
			spaceMoteThread = new Thread (new SpaceMote(this));
			spaceMoteThread.start();
		}
	}

	public void shutdown ()
	{
		if (spaceMoteThread != null)
		{
			spaceMoteThread.interrupt();
		}
	}
	...
	{
		for (int i = 0; i < keyActions.length; i++) if (keyActions[i] == gameAction) keyActions[i] = null;
		for (int i = 0; i < mouseActions.length; i++) if (mouseActions[i] == gameAction) mouseActions[i] = null;
		gameAction.reset();
	}
	...
	public List getMaps(GameAction gameCode)
	{
		...
		for (int i = 0; i < keyActions.length; i++) if (keyActions[i] == gameCode) list.add(getKeyName(i));
		for (int i = 0; i < mouseActions.length; i++) if (mouseActions[i] == gameCode) list.add(getMouseName(i));
		return list;
	}
	...
	{
		for (int i = 0; i < keyActions.length; i++) if (keyActions[i] != null) keyActions[i].reset();
		for (int i = 0; i < mouseActions.length; i++) if (mouseActions[i] != null) mouseActions[i].reset();
	}
	...
	private GameAction getMouseButtonAction(MouseEvent e)
	{
		...
		if (mouseCode != -1) return mouseActions[mouseCode];
		else return null;
	}
	...
	public synchronized void mouseMoved(MouseEvent e)
	{
		...
		if (isRecentering && centerLocation.x == e.getX() && centerLocation.y == e.getY()) isRecentering = false;
		else
		{
	...
	private void mouseHelper(int codeNeg, int codePos, int amount)
	{
		...
		if (amount < 0) gameAction = mouseActions[codeNeg];
		else gameAction = mouseActions[codePos];
		if (gameAction != null)
		{
